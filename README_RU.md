...
| **Админ** | Услуги CRUD, архив, длительность/буфер, медиа (5 фото + 1 видео), места проведения, рабочие часы, недоступные даты, записи, **архив клиентов/история**, настройки (текст и фото /start), напоминания |
...
Использование установщика разрешено только для авторизованных установок.
...
**Формат `ADMIN_IDS`** — числовые Telegram user ID (например через [@userinfobot](https://t.me/userinfobot)):
...
Также поддерживается JSON-массив: `ADMIN_IDS=[123456789,987654321]`
...
| `ATTENDANCE_CONFIRMATION_ENABLED` | `true` — подтверждение присутствия в напоминаниях |
| `ATTENDANCE_CONFIRMATION_REMINDER` | `client_1`, `client_2` или `both` |
...
8. **Настройки бота** — автоподтверждение, напоминания, контакт, язык, **🔔 Подтверждение записи** (тексты вопроса и кнопок), **👋 Стартовое сообщение** (текст и фото /start), Google Calendar
9. Проверьте полный цикл записи клиентом

### Подтверждение записи в напоминаниях

При `ATTENDANCE_CONFIRMATION_ENABLED=true` напоминания клиенту содержат кнопки **Подтверждаю** / **Нужно изменить** (нейтральные значения по умолчанию; настраиваются).

- **Подтверждаю** — статус `confirmed`, уведомление всем админам
- **Нужно изменить** — статус `cannot_attend`, уведомление админам, меню: перенести / отменить / указать причину / оставить как есть
- **Указать причину** — текст до 1000 символов, админы получают уведомление
- В списке записей админа: индикаторы ✅ / ⚠️; в карточке — статус и причина

`ATTENDANCE_CONFIRMATION_REMINDER=client_1` (по умолчанию, ~24 ч), `client_2` (~2 ч) или `both`.

Админ может вручную отправить вопрос: **📒 Записи → 🔔 Подтверждение записи** (сортировка по ближайшим записям, фильтры: сегодня / завтра / 7 дней / без ответа).

### Архив клиентов / история

В админ-панели откройте **👥 Клиенты** — кто записывался раньше, кто новый, кто повторный, полная история по каждому клиенту.

- **Фильтры:** с будущими записями, уже были, новые, повторные, отменяли, все клиенты
- **Поиск** по имени, телефону или Telegram ID
- **Карточка клиента:** статистика, будущие записи, полная история
- **✉️ Написать клиенту** — общее сообщение в Telegram (сообщение из карточки записи по-прежнему работает)
- **🔔 Подтверждение по ближайшей записи** — тот же ручной сценарий подтверждения

### Сбор данных клиента (запись)

В **⚙️ Настройки бота → 👤 Данные клиента** админ настраивает имя и телефон при записи:

- **Имя из Telegram** — `first_name` / `last_name` из профиля (без лишнего ввода)
- **Подтверждение имени** — спросить или применить автоматически
- **Телефон** — только по кнопке «Поделиться номером» или ручному вводу; бот не читает телефон сам
- **Телефон обязателен / необязателен** — можно пропустить, если необязателен

Профиль (`username`, `language_code`, `last_seen_at`, источник телефона) хранится в локальной SQLite. Не коммитьте `.env` и файлы базы.

### Настраиваемые тексты подтверждения записи

В **⚙️ Настройки бота → 🔔 Подтверждение записи** админ настраивает по языкам (RU/EN):

- Заголовок и вопрос в автоматических напоминаниях и при ручной отправке
- Подписи кнопок Да/Нет
- Ответ клиенту после подтверждения
- Уведомления админу при подтверждении или запросе изменений
- Предпросмотр с неактивными кнопками; сброс к стандарту по языку

Один и тот же текст используется в напоминаниях, ручной отправке и предпросмотре.

### Тестовый режим напоминаний и диагностика

Повторные напоминания блокируются **полями datetime** в таблице `bookings` (NULL = ещё не отправлено):

| Колонка | Назначение |
|---------|------------|
| `client_reminder_1_sent_at` | Первое напоминание клиенту (или тестовое в test mode) |
| `client_reminder_2_sent_at` | Второе напоминание клиенту |
| `admin_reminder_sent_at` | Напоминание админу |

Устаревшие булевы колонки (`reminder_24h_sent`, `reminder_2h_sent`, `admin_reminder_sent`) могут остаться в старых БД, но **планировщик их не использует**.

**Не используйте** несуществующие имена вроде `client_reminder_1_sent`. Вместо этого:
...
Пример SQLite (правильные имена колонок):
...
В **⚙️ Настройки бота → 🧪 Тестовый режим** — кнопка **Отправить тестовое напоминание по ближайшей записи** для мгновенной проверки доставки в Telegram.
...
Этот проект является проприетарным. Нельзя копировать, изменять, распространять, продавать или использовать это программное обеспечение без предварительного письменного разрешения правообладателя. Подробности см. в файле [LICENSE](LICENSE).
