...
from app.services.admin_bookings_service import is_manual_attendance_send_eligible, parse_attendance_back
...
async def admin_attendance_send(callback: CallbackQuery, is_admin: bool, lang: str) -> None:
    ...
    async with async_session_factory() as session:
        ...
        if not booking or not is_manual_attendance_send_eligible(booking):
            await edit_or_send(callback, t(lang, "not_found"))
            return
...
async def _do_send_question(
    ...
    async with async_session_factory() as session:
        ...
        if not client or not client.telegram_id:
            await edit_or_send(callback, t(lang, "admin_attendance_send_failed"))
            return
        ...
        try:
            ...
            logger.exception("Manual attendance send failed for booking_id=%s", booking_id)
            sent = False
        if not sent:
            await edit_or_send(callback, t(lang, "admin_attendance_send_failed"))
            return
